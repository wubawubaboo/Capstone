        ...
        <div className="fixed inset-0 flex items-center justify-center bg-slate-900 bg-opacity-60 z-50 p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto border border-slate-200 flex flex-col">
                    ...
                    <div className="space-y-4">
                        ...
                        {report.attachment_path && (
                            <div>
                                ...
                                <a
                                    href={route('vawc.reports.attachment', report.id)}
                                    target="_blank"
                                    rel="noreferrer"
                ...
                <div className="p-4 border-t bg-slate-50 flex justify-end">
                    <button
                        onClick={onClose}
                        className="bg-[#3B122D] text-white px-6 py-2 rounded font-bold hover:bg-[#280c1e] transition"
                    >
                        CLOSE
                    </button>
                </div>
...
export default function Reports({ reports, filters }) {
    ...
    const handleStatusChange = (reportId, newStatus) => {
        router.put(route('vawc.reports.update-status', reportId), {
            status: newStatus
        }, { preserveScroll: true });
    };
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="relative">
                        <select
                            ...
                            className="appearance-none bg-white border border-slate-200 text-slate-700 text-sm rounded-md pl-4 pr-10 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-rose-800 focus:border-rose-800 cursor-pointer hover:bg-slate-50 transition-colors duration-200"
                        >
                                    ...
                                    <tr key={report.id} className={isCritical ? 'bg-red-50' : 'hover:bg-rose-50/20'}>
                                        ...
                                        <td className="py-4 px-2 text-center">
                                            <div className="flex justify-center gap-2">
                                                ...
                                                <Link
                                                    href={route('vawc.blotters.create', { report_id: report.id })}
                                                    className="bg-[#3B122D] text-white px-3 py-1.5 rounded text-xs font-bold hover:bg-[#280c1e] transition"
                                                >
                                                    LOG TO VAWC
                                                </Link>
                                            </div>
