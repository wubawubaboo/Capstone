...
export default function BlotterManagement() {
    const records = [
        { id: '#VA-007', complainant: 'Maria Aquino', type: 'Assault', status: 'Scheduled', badgeColor: 'bg-amber-100 text-amber-800' },
    ];
            ...
            <div className="bg-white rounded-lg p-6 shadow-sm border border-slate-200">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                    <div>
                        ...
                        <p className="text-sm text-slate-500">Manage and review all reported VAWC incidents.</p>
                    </div>
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                        ...
                        />
                        <button className="bg-[#0a2540] text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-slate-800 transition whitespace-nowrap">
                            + New Blotter
                        </button>
                    </div>
                </div>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {records.map((row) => (
                                <tr key={row.id} className="hover:bg-slate-50 transition">
                                    ...
                                    <td className="py-3 px-4">
                                        <Link
                                            href={`/vawc/blotter/${row.id.replace('#', '')}/history`}
                                            className="bg-[#0a2540] text-white px-5 py-1.5 rounded text-xs font-medium hover:bg-slate-800 inline-block"
                                        >
                                            View
                                        </Link>
                                    </td>
